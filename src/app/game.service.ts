...
export class GameService {
  ...
  loadeStateFromLS() {
    this._gameState = JSON.parse(window.localStorage.getItem("gameState"));
    this.prizeState$.next([...this._gameState.prizeState]);
    this.boardState$.next([...this._gameState.boardState]);
  }

  saveStateToLS() {
    window.localStorage.setItem("gameState", JSON.stringify(this._gameState));
  }
  ...
  applyPrizeState(prize) {
    this._prizeState.splice(this._prizeState.findIndex(e => {
      return e.label === prize.label;
    }),1,prize);
    this._gameState = new GameState(this._boardState, this._prizeState);
    this.saveStateToLS();
  }
